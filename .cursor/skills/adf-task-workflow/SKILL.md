...
Before creating or splitting tasks, read [references/responsibility-unit.md](references/responsibility-unit.md). Policy SSOT node: `adf.task-policy.responsibility-unit` (v1.0).

- One task = one primary deliverable, one `작업 유형`, one close-out gate.
- Split design / skill / implementation / verification into a task chain with `선행 작업`, `후행 작업`, and `종속성`.
- Link `목표` to the direct leading KPI only; document parent outcome KPI in `종속성` and Goal `상위 목표`.
...
3. Fetch the Tasks DB schema if creating or updating task rows.
4. For user-named task IDs, fetch the row. Otherwise query `상태 in (진행중, 대기)` sorted by `우선순위` / `작업 ID` and confirm the selected task.
5. Set `상태 = 진행중` only when starting work and blocking `선행 작업` are `완료`. Set `진행일` as a datetime (`date:진행일:is_datetime=1`, ISO-8601) at pickup. Use `보류` for blockers. Do not set `완료` unless the user explicitly asks.
6. Link every non-trivial task to at least one `목표` (direct leading KPI). If the work does not serve an existing goal, create or request a goal before adding task sprawl.
...
9. Report: task ID, status change, linked goal, linked nodes/edges, dependency chain, next unblocked task.
